...
def load_house_dataset(datadir):
    ...
    trainX = torch.from_numpy(trainX.to_numpy(dtype=float)).float()
    trainY = torch.from_numpy(trainY.to_numpy(dtype=float)).float()
    testX = torch.from_numpy(testX.to_numpy(dtype=float)).float()
    testY = torch.from_numpy(testY.to_numpy(dtype=float)).float()
