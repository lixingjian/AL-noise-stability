...
def test(models, criterion, method, dataloaders, mode='val', regression=False):
    ...
    with torch.no_grad():
        ...
        for (inputs, labels) in tqdm(dataloaders[mode], total=len(dataloaders[mode])):
...
def train_epoch(models, method, criterion, optimizers, dataloaders, epoch, epoch_loss, regression, class_balance):
    ...
    if class_balance:
        ...
        class_weight = 1.0 / (np.array(label_cnt) + 1e-5)
    ...
    for data in dataloaders['train']:
        ...
        if method == 'lloss':
            ...
            #sample_weight = torch.ones(inputs.shape[0]).to(device)
